...
The [start_server_stack.sh](server/start_server_stack.sh) script will start up these services as Docker containers.

The configuration for InfluxDB and Granfana is in [server_stack.env](server/server_stack.env). Change this accordingly before you run the start script. These parameters will be used in the instructions below.
